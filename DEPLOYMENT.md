...
0.  **Quality First:** All code must be formatted and linted before it is even considered for build.
1.  **Safety First:** Never push code that hasn't been visually verified in a production-like environment.
2.  **Source of Truth First:** GitHub is the definitive record. Code must live in version control _before_ it lives on the production server.
...
The script will guide you through four mandatory phases:

#### PHASE 0: Code Quality (The "Stage Prep")

- **Formatting:** Runs `npm run format` (Prettier) to ensure visual consistency.
- **Linting:** Runs `npm run lint` (ESLint) to catch logical errors and broken textiles.
- **The Check:** If linting fails, the deployment stops. You must fix the code quality before proceeding.

#### PHASE 1: Build & Verify (The "Dress Rehearsal")

- Runs `npm run build` to generate the `dist/` folder.
- Runs `npm run preview` to host the _compiled_ version locally.
...
- **Linting Compliance:** Always run `npm run lint` before suggesting a final code change.
- **VS Code Integration:** Ensure the `.vscode/settings.json` is respected so "Format on Save" handles Prettier automatically.
